...
class Ping(Axon.Component.component):
    ...
    def __init__(self, frequency=Frequency("PingFreq", period=60)):
    ...
    def main(self):
        ...
        while True:
            ...
            if self.dataReady("inbox"):
                ...
                self.deviation.append(self.lastping + self.frequency.Period() - now)
                arg = {}
                if self.verbosity > 0:
                ...
                if self.verbosity > 2:
                    meandeviation = (fsum(self.deviation[10:]) / 10, fsum(self.deviation[20:]) / 20, fsum(self.deviation) / 30)
                    arg['deviation'] = self.deviation
                    arg['meandeviation'] = meandeviation
                    arg['time'] = time()
                response = Message(sender=self.name, recipient="ALL", func="PING", arg=arg)
                self.lastping = time()
            if response:
